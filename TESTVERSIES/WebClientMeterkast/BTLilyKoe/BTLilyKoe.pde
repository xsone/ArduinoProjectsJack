...
void setup()
{
 ...
 for(;;)
 {
  ...
  if(staartHoog==1 && hoogGeweest==0) 
   {
    delay(tijd);
    if(ligtRechts==1 || ligtLinks==1)phone.sendTxt("0649982176","L"); //mySerial.println("Ligt");
    phone.sendTxt("0649982176","H"); //mySerial.println("Staartomhoog");
  ...
  if(staartLaag)
   ...
   if(digitalRead(2)==HIGH&&hoog==0) // test op sensorinput 2
   {
    ...
    //if(ligtRechts=1&&staartHoog=1) mySerial.println("Rechterzij + Staartomhoog");
    //if(ligtRechts=1&&staartLaag=1) mySerial.println("Rechterzij + Staartomlaag");
    //if(ligtLinks=1&&staartHoog=1) mySerial.println("Linkerzij + Staartomhoog");
    //if(ligtLinks=1&&staartLaag=1) mySerial.println("Linkerzij + Staartomlaag");
