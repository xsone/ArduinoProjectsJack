/*
 Webmonitor voor:
 - Besturing Buffervat o.b.v. aanbod zonne energie
 created april 2020, door Jack Cop
 */
#include <Ethernet.h>
#include <SPI.h>             // needed for Arduino versions later than 0018

byte mac[] = {0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED }; //physical mac address
byte ip[] = {192, 168, 178, 102 }; // ip in lan
byte gateway[] = {192, 168, 178, 80 }; // internet access via router
...
void setup() 
{
  ...
  Serial.begin(9600);
  Serial.println("Energieopbrengst Monitor Programma gestart v1.2...");
} 
...
void loop()
{
  ...
  //EthernetClient client = meterkastServer.available();
  if (client) {
    boolean currentLineIsBlank = true;
    Serial.println("De meterkast client meld zich..");
    ...
    while (client.connected()) {
        if (client.available()) {
         ...
         /*
           ...
           // send a standard http response header
              ...
              client.println("<center><p><h1><align=center>Energylogger & Heatingcontrol Webserver  v1.7  </h1></p> ");
              ...
              client.println("<TR ALIGN=center><TD>" + String(cvTimer) + "</TD><TD>" + String(Temperature) + " &#8451" + "</TD><TD>" + String(Setpoint) + " &#8451" + "</TD><TD>" + String(Humidity) + " % RH" + "</TD><TD>" +  String(CVstate) +"</TD><TD>" +  String(Move) + "</TD><TD>" + String(Smoke) + "</TD></TR>");
        ...
        //else if (c != '\r') currentLineIsBlank = false; 
         ...
         if (httpData.substring(5,15) == "?cmd=CVaan") 
          ...
          if (cvTimer >= MAX_TIME_CV_ON) 
           { 
            
            CVstate = "UIT";
            //client.println("HTTP/1.1 200 OK");
            //client.println("Content-Type: application/json");  // JSON response type
            //client.println("</center><br><p> (c) CV uit MAX Timer <a href=\"http://217.120.27.59>www.sens4x.com<a>");
            //client.println("Content-Type: text/html");
            //client.println("Connection: close");
            //client.println();
            //client.print("{\"res\":\"OK\",\"cvstate\":\""); client.print(CVstate);
